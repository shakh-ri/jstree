...
$(function () {
    ...
    const parseDataFromApi = (data) => {
        ...
        const parseSection = (section, sort) => {
            ...
            if (children) {
                if (children.sections && children.sections.length) {
                    result.children.push(...children.sections.map((childSection, index) => {
                        return parseSection(childSection, index);
                    }));
                }

                if (children.content && children.content.length) {
                    result.children.push(...children.content.map((childContent, index) => {
                        return parseContent(childContent, index);
                    }));
                }

                if (children.messages && children.messages.length) {
                    result.children.push(...children.messages.map((childMessage, index) => {
                        return parseMessage(childMessage, index);
                    }));
                }
            }
            return result;
        ...
        const parseMessage = (message, sort) => {
            return {
                id: message.id,
                text: message.title,
                sort: sort,
        ...
        const parseContent = (content, sort) => {
            return {
                id: content.id,
                text: content.title,
                sort: sort,
        ...
        const root = parseSection(data.data.root, 0);

        const unattachedSections = data.data.unattached_sections.map((section, index) => {
    ...
    const createJsTree = (data) => {
        $('#JsSсheme').jstree({
            ...
            'types': {
                ...
                "section": {
                    "valid_children": ["message", "content", "section"],
                },

            },
            'plugins': ["search", "dnd", "contextmenu", "types", "show_matches_children", "html_data", "ui", "crrm", "sort"],
            'search': {
            ...
            'sort': function(a, b) {
                ...
                const aNode = a1.original;
                const bNode = b1.original;
                ...
                if (aNode.type === bNode.type) {
                    return (aNode.sort < bNode.sort) ? 1 : -1;
                } else {
                    return (aNode.type < bNode.type) ? 1 : -1;
                }
        ...
        function isDraggable (node) {
            const editModeOff = document.querySelector('.status.normal');
            const type = node[0].original.type;
            const parent = node[0].parent;
            ...
            var items = {
                ...
                "moveTounattached": {
                    ...
                    "_disabled": function (obj) {
                        ...
                        if (parent === UNATTACHED_CONTENT_ID ||
                            parent === UNATTACHED_MESSAGES_ID ||
                            parent === UNATTACHED_SECTIONS_ID) {
                            return true
                        }
    ...
    // $('JsScheme').on('dnd_stop.vakata', function (e, data) {
    //     $('JsScheme').jstree(true).sort(root, true);
    //     $('JsScheme').jstree(true).redraw_node(root, true);
    // });
